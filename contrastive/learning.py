...
class ContrastiveLearner(acme.Learner):
  ...
  def __init__(
    ...
    def critic_loss(q_params,
      ...
      # jax.debug.print('critic logits: {logits}', logits=logits)
      if config.use_td:
    ...
    def l_loss(l_params, 
      ...
      # jax.debug.print('l_loss: {l}, {g}', l=language.shape, g=goal.shape)
      ...
      # jax.debug.print('language logits: {logits}', logits=logits)      
      loss = loss_fn(logits)
      loss = jnp.mean(loss)
    ...
    l_grad = jax.value_and_grad(l_loss)
    ...
    def update_step(
      ...
      if config.language:
        language = transitions.extras['language']
        goal = transitions.extras['goal']
        l_data = (language, goal)
      # jax.debug.print('in update step, language is {x}', x=language)

      # jax.debug.print('PRINTING TRANSITIONS')
      # jax.debug.print('{x}', x=transitions.observation)

      # jax.debug.print('l_data is: {x}', x=l_data)
    ...
    # Iterator on language descriptions.
    self._l_iterator = l_iterator

    update_step = utils.process_multiple_batches(update_step,
                                                 config.num_sgd_steps_per_step)
    # Use the JIT compiler.
    if config.jit:
      # jax.debug.print('jitting update')
      self._update_step = jax.jit(update_step)
    else:
      # jax.debug.print('not jitting update')
      self._update_step = update_step

    def make_initial_state(key):
      """Initialises the training state (parameters and optimiser state)."""
      key_policy, key_q, key_l, key = jax.random.split(key, 4)
  ...
  def step(self):
    with jax.profiler.StepTraceAnnotation('step', step_num=self._counter):
      ...
      if self._l_iterator:
        language, goal = next(self._l_iterator)
        # jax.debug.print('language is {x}', x=language)
        # Create a new Transition object with l_data in extras
        new_extras = dict(transitions.extras) if transitions.extras else {}
        new_extras['language'] = jnp.array(language)
        new_extras['goal'] = jnp.array(goal)
        transitions = transitions._replace(extras=new_extras)
      # print(f'new_transitions extras: {new_transitions.extras}')
      # jax.debug.print('new transition observation shape: {obs}', obs=new_transitions.observation.shape)
