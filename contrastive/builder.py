...
def create_maze_dataset_iterator(config):
    """ Create an infinite dataset for language descriptions of a maze """
    maze_shape = config.maze_shape
    rows, cols = maze_shape
    total_dim = rows + cols

    @tf.function
    def generate_single_item(unused_):
        # Randomly choose row or column
        index = tf.random.uniform((), maxval=total_dim, dtype=tf.int32)
        
        # Create one-hot encoding for both rows and columns
        one_hot = tf.one_hot(index, total_dim)
        
        row = tf.cond(
            index < rows,
            lambda: tf.cast(index, tf.float32),
            lambda: tf.random.uniform((), maxval=rows, dtype=tf.float32)
        )
        
        col = tf.cond(
            index < rows,
            lambda: tf.random.uniform((), maxval=cols, dtype=tf.float32),
            lambda: tf.cast(index - rows, tf.float32)
        )
        
        # Add uniform noise within the cell
        row_noise = tf.random.uniform(())
        col_noise = tf.random.uniform(())
        point = tf.stack([row + row_noise, col + col_noise]) * config.scaling_factor
        
        return one_hot, point

    # Create a dataset of indices
    dataset = tf.data.Dataset.range(1).repeat()

    # Map the indices to our data generation function
    dataset = dataset.map(
    ...
    dataset = dataset.map(
        lambda x, y: (tf.ensure_shape(x, [total_dim]), tf.ensure_shape(y, [2])),
        num_parallel_calls=tf.data.experimental.AUTOTUNE
    )
    ...
    # Batch the dataset
    dataset = dataset.batch(config.batch_size * config.num_sgd_steps_per_step, drop_remainder=True)
...
class ContrastiveBuilder(builders.ActorLearnerBuilder):
  ...
  def make_learner(
    ...
    # Language additions
    # print(f'BATCH SIZE: {self._config.batch_size}')
    l_optimizer = optax.adam(learning_rate=self._config.learning_rate, eps=1e-7)
    if self._config.language:
      jax.debug.print('scaling factor = {x}', x=self._config.scaling_factor)
      l_iterator = create_maze_dataset_iterator(config=self._config)
    else:
      l_iterator = None
